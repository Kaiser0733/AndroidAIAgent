...
class ToolRegistry {
    ...
    fun describeForPrompt(): String {
        ...
        return buildString {
            ...
            appendLine("  You call: press_enter {}  (submits the search — type_text only fills the field, it does NOT submit)")
            appendLine("    → result: 'OK press_enter (ACTION_IME_ENTER on focused field)'")
            appendLine("  You call: wait_seconds {\"seconds\":2}  (let results load)")
            ...
            appendLine()
            appendLine("CRITICAL v0.6.6 RULE — type_text does NOT submit:")
            appendLine("type_text uses ACTION_SET_TEXT which bypasses the keyboard. The IME's")
            appendLine("Search/Go button is NOT pressed. You MUST call press_enter after type_text")
            appendLine("in a search box to actually run the search. Never claim 'I searched for X'")
            appendLine("if you have not called press_enter — that is a hallucination. The text is")
            appendLine("just sitting in the input field, the search has not been executed.")
            appendLine()
            ...
            appendLine("If press_enter returns ERROR, fall back to: read_screen, then tap_text on the")
            appendLine("submit button (look for 'Search', 'Go', or a magnifying glass icon).")
            appendLine()
